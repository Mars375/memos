...
def test_tools_list(mem):
    assert len(TOOLS) == 9  # 4 memory tools + 3 kg tools + 2 context-stack tools (P7)
...
async def test_http_tools_list(mem):
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        ...
        assert len(r.json()["result"]["tools"]) == 9  # 4 memory tools + 3 kg tools + 2 context-stack tools (P7)
