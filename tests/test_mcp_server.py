...
def test_tools_list(mem):
    assert len(TOOLS) == 11  # 4 memory + 3 kg + 2 decay/reinforce + 2 context-stack (P7)
...
async def test_http_tools_list(mem):
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        ...
        assert len(r.json()["result"]["tools"]) == 11  # 4 memory + 3 kg + 2 decay/reinforce + 2 context-stack
